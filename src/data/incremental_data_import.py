...
def import_historic_csv_files(url, account_name, account_key, container_name):
    ...
    blobs_already_downloaded = get_blob_names_already_imported(block_blob_service, container_name)
    ...
    for csv_link in historic_csv_links:
        ...
        if not filename in blobs_already_downloaded:
            # read csv file content and create blob from it
            csv = requests.get(csv_link)
            block_blob_service.create_blob_from_text(storage_container_name, filename, csv.text)
